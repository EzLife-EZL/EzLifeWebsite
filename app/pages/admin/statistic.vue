...
onMounted(async () => {
  loading.value = true
  try { data.value = await $fetch('/api/stats/monthly') }
...
const webBar = computed<ChartData<'bar'>>(() => ({
  labels: labels.value,
  datasets: [
    { label: 'Basic (Web)', data: (data.value?.subs ?? []).map((m:any)=> m.web.basic) },
    { label: 'Plus (Web)',  data: (data.value?.subs ?? []).map((m:any)=> m.web.plus)  },
    { label: 'VIP (Web)',   data: (data.value?.subs ?? []).map((m:any)=> m.web.vip)   },
  ]
}))
...
const appBar = computed<ChartData<'bar'>>(() => ({
  labels: labels.value,
  datasets: [
    { label: 'Basic (App)', data: (data.value?.subs ?? []).map((m:any)=> m.app.basic) },
    { label: 'Plus (App)',  data: (data.value?.subs ?? []).map((m:any)=> m.app.plus)  },
    { label: 'VIP (App)',   data: (data.value?.subs ?? []).map((m:any)=> m.app.vip)   },
  ]
}))
...
const webappBar = computed<ChartData<'bar'>>(() => ({
  labels: labels.value,
  datasets: [
    { label: 'Basic (WebApp)', data: (data.value?.subs ?? []).map((m:any)=> m.web.basic + m.app.basic) },
    { label: 'Plus (WebApp)',  data: (data.value?.subs ?? []).map((m:any)=> m.web.plus  + m.app.plus ) },
    { label: 'VIP (WebApp)',   data: (data.value?.subs ?? []).map((m:any)=> m.web.vip   + m.app.vip  ) },
  ]
}))
...
const revenueLine = computed<ChartData<'line'>>(() => ({
  labels: labels.value,
  datasets: [
    { label: 'Doanh thu (VND)', data: (data.value?.revenue ?? []).map((r:any)=> r.revenue) }
  ]
}))
...
<template>
  <div class="page-wrap">
    ...
    <div class="dash-card">
      ...
      <div class="chart-box"><Line :data="revenueLine" :options="lineOpts" /></div>
      <div class="note">Doanh thu gần nhất: <b>{{ money(data?.revenue?.at(-1)?.revenue ?? 0) }}</b></div>
    </div>
  </div>
</template>
